// window.addEventListener("DOMContentLoaded", function () {}); 이게있어야 콘솔보임
...
$(function () {
  ...
  // 2. 아이디 선택자
  // 자바스크립트 방식 ->가 id1인 글씨 색깔(color) 변경 (색상은 자유롭게)
  ...
  $(".item").css({ color: "green", "background-color": "yellow" });
  ...
  /* h3 이면서 클래스가 ch -> color : violet */
  $("div h3").css("background-color", "khaki");
  /*  h3 공통 : background- color: khaki */
  ...
  // checkbox 에 change 이벤트 핸들러 등록
  $("#div3 :checkbox").change(function () {
    console.log(this);
    console.log($(this));
    let checkbox = $(this);
    console.log(checkbox.prop("checked")); //ture or false
    console.log(checkbox.attr("checked"));
  });
  // select 에 change 이벤트 핸들러 등록
    $("#national").change(funciton () {
    // select 태그의 option 태그 중 선택된 
    let value = $("select > option :selected").val();
    $("#result").val(value);
    // 아이디가 result인 input 값으려 추가
    });
});
